<div x-data="{
    ...
    closeAll() { this.showModal = false; this.resetForm(); },
    resetForm() { this.photoPreview = null; this.form = { id_local: '', id_apiario: '', nombre: '', precio: '', observaciones: '', foto_b64: null }; }
}" 
...
@offline.window="isOnline = false">
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            ...
            <x-filter-menu :hasActiveFilters="$hasFilters">
                ...
                <div>
                    ...
                    <input wire:model.live.debounce.300ms="search" type="text" placeholder="Ej: Miel 500g..." class="w-full text-sm border-gray-300 rounded-md focus:ring-yellow-500">
                </div>
                <div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white rounded-2xl shadow-lg border {{ $prod->estado_activo ? 'border-yellow-200' : 'border-gray-300 opacity-75' }} overflow-hidden flex flex-col hover:shadow-xl transition">
            <!-- Imagen Base64 -->
            <div class="h-48 w-full bg-gray-100 flex items-center justify-center relative">
                ...
                @endif
                <div class="absolute top-2 right-2 bg-black/60 text-white text-xs px-2 py-1 rounded-md font-bold backdrop-blur-sm">
            ...
            <div class="p-4 flex-1 flex flex-col">
                ...
                <div class="grid grid-cols-2 gap-2 border-t border-gray-100 pt-3 mt-4">
                    <button @click="openEdit({{ $prod->toJson() }})" class="py-1.5 text-xs font-bold rounded border border-blue-200 text-blue-600 hover:bg-blue-50">✏️ Editar</button>
                    <button wire:click="toggleStatus({{ $prod->id }})" class="py-1.5 text-xs font-bold rounded border {{ $prod->estado_activo ? 'border-red-200 text-red-600 hover:bg-red-50' : 'border-green-200 text-green-600 hover:bg-green-50' }}">
                        {{ $est->estado_activo ?? true ? 'Ocultar' : 'Activar' }}
                    </button>
                </div>
            </div>
        </div>
    ...
    <!-- MODAL -->
    <div wire:ignore.self x-show="showModal" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm" style="display:none;" x-transition>
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden relative" @click.away="closeAll()">
            ...
            <form @submit.prevent="save()" class="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
                ...
                <div class="flex flex-col items-center gap-2">
                    <div class="w-32 h-32 rounded-xl bg-gray-100 border-2 border-dashed border-gray-300 overflow-hidden flex items-center justify-center relative">
                        <template x-if="photoPreview"><img :src="photoPreview" class="w-full h-full object-cover"></template>
                        <template x-if="!photoPreview"><span class="text-gray-400 text-sm font-bold text-center">Subir<br>Foto</span></template>
                        <input type="file" accept="image/*" @change="handleFile($event)" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer">
                    </div>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700">Nombre del Producto</label>
                    <input type="text" x-model="form.nombre" required class="mt-1 w-full border-gray-300 rounded-md focus:ring-yellow-500">
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Precio (COP)</label>
                        <input type="number" x-model="form.precio" required class="mt-1 w-full border-gray-300 rounded-md focus:ring-yellow-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Apiario de Origen</label>
                        <select x-model="form.id_apiario" required class="mt-1 w-full border-gray-300 rounded-md focus:ring-yellow-500">
                ...
                <div>
                    ...
                    <textarea x-model="form.observaciones" rows="2" class="mt-1 w-full border-gray-300 rounded-md focus:ring-yellow-500"></textarea>
                </div>

                <button type="submit" class="w-full bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-3 rounded-xl transition">Guardar</button>
            </form>
        </div>
    </div>
</div>