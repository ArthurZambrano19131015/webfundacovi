...
<div x-data="{ isOnline: navigator.onLine }" @online.window="isOnline = true" @offline.window="isOnline = false"
    ...
    <div class="shrink-0 w-full h-40 md:h-56 bg-cover bg-center flex items-center shadow-md relative border border-yellow-600/30 overflow-hidden"
    ...
    <div class="flex-1 mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6 lg:overflow-hidden px-4 md:px-0">
        ...
        <div
            class="bg-[#E9EFA7]  shadow-lg border border-yellow-300 flex flex-col h-full overflow-hidden relative">
            <div class="h-24 w-full bg-cover bg-center shrink-0 opacity-80"
            ...
            <div class="px-5 flex items-end gap-3 -mt-10 mb-3 shrink-0 relative z-10">
                <img src="{{ $user->foto ? asset('storage/' . $user->foto) : asset('img/default-avatar.png') }}"
                    alt="Foto"
                    class="w-20 h-20 border-4 border-[#E9EFA7] shadow-sm object-cover bg-white">
                <div class="pb-1 overflow-hidden">
                    <h2 class="text-xl font-black text-gray-900 leading-none truncate">{{ $user->nombre_completo }}</h2>
                    <span
                        class="inline-block bg-yellow-400 text-yellow-900 text-[10px] px-2 font-bold mt-1 uppercase tracking-wider">
            ...
            <div class="px-4 pb-4 flex-1 grid grid-cols-2 gap-3 content-center">
                <div class="bg-white/60 p-3 shadow-sm border border-white/50 h-full flex flex-col">
                    ...
                    @else
                        <p class="text-[11px] text-red-600 font-bold ml-6">Sin apiarios. <a
                                href="{{ route('admin.apiarios') }}" class="underline">¡Registra uno!</a></p>
                    @endif
                </div>
                ...
                <div class="bg-white/60 p-3 shadow-sm border border-white/50 h-full flex flex-col">
                    ...
                    @if ($apiariosCount > 0)
                        <p class="text-[11px] text-gray-700 ml-6 leading-tight">Zonas: <span
                ...
                <div
                    class="bg-white/60 p-3 shadow-sm border border-white/50 h-full flex flex-col col-span-2">
                    <div class="flex items-center gap-2 mb-1">
                        <span class="text-base">🍯</span>
                    ...
                    @if ($miProduccion > 0)
                        ...
                        <p class="text-[10px] font-bold text-gray-500 mt-1 ml-6 uppercase">Aportas el <span
                                class="text-yellow-600">{{ $porcentajeAporte }}%</span> del total.</p>
                    @else
                        <p class="text-[11px] text-yellow-600 font-bold ml-6">0.0 kg. <a
                                href="{{ route('apicultor.cosechas') }}" class="underline">¡Registra cosecha!</a></p>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="bg-white shadow-lg border border-gray-200 overflow-hidden flex flex-col relative h-64 lg:h-full z-10"
            x-data="{
                initMap() {
            ...
            <div class="absolute top-0 w-full flex justify-center z-20 pointer-events-none">
                <div
                    class="bg-blue-600 text-white font-black text-[10px] uppercase tracking-widest px-6 py-1 shadow-md">
                    Ubicación Regional
                </div>
            </div>

            <template x-if="isOnline">
                <div id="mapa-apiarios" class="w-full h-full z-10 overflow-hidden"></div>
            </template>

            <template x-if="!isOnline">
                <div
                    class="w-full h-full bg-gray-100 flex flex-col items-center justify-center p-6 text-center z-10 overflow-hidden">
        ...
        <div x-data="{
            ...
            class="bg-lime-800 shadow-lg border border-lime-900 flex flex-col h-64 lg:h-full relative overflow-hidden">

            <div class="absolute top-0 w-full flex justify-center z-10">
                <div
                    class="bg-yellow-500 text-white font-black text-[10px] uppercase tracking-widest px-6 py-1 shadow-md">
                    Catálogo Comercial
                </div>
            </div>
                ...
                @if ($productos->count() > 0)
                    <div x-ref="slider"
                        ...
                        @foreach ($productos as $prod)
                            <!-- Tarjeta de Producto:  -->
                            <div
                                class="shrink-0 snap-center w-28 bg-white shadow-lg overflow-hidden flex flex-col group cursor-pointer border border-lime-700">

                                <div
                                    class="h-24 w-full bg-lime-50 relative overflow-hidden flex items-center justify-center">
                                    @if ($prod->foto)
                                        <!-- Aseguramos que la imagen llene sin desbordar -->
                                        <img src="{{ $prod->foto }}"
                                    ...
                                    <div
                                        ...
                                        <span
                                            class="text-white text-sm font-black">${{ number_format($prod->precio, 0) }}</span>
                                    </div>
                                </div>

                                <div class="p-2 flex-1 flex items-center justify-center text-center bg-white">
                                    <h3 class="text-[11px] font-bold text-gray-800 leading-tight line-clamp-2">
                                        {{ $prod->nombre }}</h3>
                                </div>
                            </div>
                        @endforeach
            ...
            <div class="p-4 w-full shrink-0 z-10 relative">
                @if (auth()->user()->role->nombre_rol === 'Administrador')
                    <a href="{{ route('inicio') }}#productos"
                        class="block text-white text-[10px] font-bold bg-black/30 hover:bg-black/50 transition text-center py-2 shadow-inner uppercase tracking-wider">
